...
// ❌ const { serverUrl } = useContext(authDataContext); // Hata diya gaya hai
import Post from "../components/Post";
import AIChat from "../components/AIChat";

// Mock celebrity data for suggested users (Optional Fallback)
...
function Home() {
  // ✅ Changed: userData ko default empty object rakha gaya hai taaki properties padhi ja sakein
  ...
  const handleSuggestedUsers = async () => {
    try {
      ...
      // Ensure result.data is an array or default to empty array
      const realUsers = Array.isArray(result.data) ? result.data : [];
      const mixedSuggestions = [...celebritySuggestions.slice(0, 3), ...realUsers.slice(0, 2)];
      
      setSuggestedUser(mixedSuggestions);
    } catch (error) {
      console.log("Error fetching suggested users:", error); // Log the specific error
      ...
      <div className="w-full lg:w-[25%] flex flex-col gap-5 mt-[90px] overflow-y-auto custom-scrollbar">
        ...
        <div className={`${cardClasses} rounded-2xl p-6 shadow-2xl flex flex-col items-center relative
          ...
          <h2 className="mt-4 font-bold text-xl bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
            {/* ✅ FIX: Optional Chaining and Fallback for "undefined undefined" */}
            {`${userData?.firstName || 'Guest'} ${userData?.lastName || 'User'}`}
          </h2>
          <p className="text-sm text-gray-300 text-center mt-1">{userData?.headline || 'Not Logged In'}</p>
...
<div className={`${secondaryCardClasses} rounded-2xl p-5 shadow-lg hover:shadow-xl transition-shadow duration-300`}>
  ...
  <div className="flex flex-col gap-3 max-h-[500px] overflow-y-auto custom-scrollbar">
    {/* ✅ Ensures suggestedUser is an array before mapping */}
    {Array.isArray(suggestedUser) && suggestedUser.length > 0 ? (
      suggestedUser.map((su) => (
        <div
          key={su.userName} // ✅ Using unique userName as key
      ...
      <div className="w-full lg:w-[50%] flex flex-col gap-5 mt-[90px] overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-4">
            {/* ✅ Final FIX: Checking if postData is a valid array before mapping */}
          {postData && Array.isArray(postData) && postData.map((post, index) => (
            <div key={post._id || index} className={`${secondaryCardClasses} rounded-2xl p-6 shadow-lg
