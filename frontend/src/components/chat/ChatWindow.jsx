...
function ChatWindow() {
  ...
  const fetchChatHistory = async (otherUserId) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/chat/history/${otherUserId}`);
  ...
  const checkCallAvailability = async (targetUserId) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/connection/getStatus/${targetUserId}`);
  ...
  const getAgoraToken = async (channelName) => {
    try {
      const response = await axios.post('http://localhost:8000/api/agora/token', {
        channelName,
        userId: userId
      });
